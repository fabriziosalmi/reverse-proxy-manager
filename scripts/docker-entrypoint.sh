...
if [ ! -f "instance/italiacdn.db" ]; then
...
ADMIN_COUNT=$(flask shell -c "from app.models.models import User; print(User.query.filter_by(role='admin').count())" 2>/dev/null || echo "0")
